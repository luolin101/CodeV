...
def add_all_steps_with_analysis(data_file_path,step1_file_path,step2_file_path,step2_analysis_file_path,step3_file_path):
    ...
    for data in dataList:
        ...
        save_data_list.append(data)
    with open(f"data_after_all_filter_v1.json", 'w', encoding='utf-8') as outfile:
        json.dump(save_data_list, outfile, indent=4)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Script configuration")
    parser.add_argument("--in_folder", type=str, help="The output folder for results")
    args = parser.parse_args()
    in_folder = args.in_folder
    add_all_steps_with_analysis("Visual SWE-bench/multi_data_onlyimage.json.json",f"{in_folder}/step1.json",f"{in_folder}/step2_des.json",f"{in_folder}/step2_analysis.json",f"{in_folder}/step3.json")
